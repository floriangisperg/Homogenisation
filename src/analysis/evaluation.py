...
def calculate_dna_metrics(df_dna_pred: pd.DataFrame) -> Dict[str, float | int]:
    ...
    Excludes 'resuspended biomass' step. Handles NaNs. Includes NRMSE.
    """
    obs_col = "dna_conc"
    pred_col = "dna_pred"
    step_col = "process_step"

    if obs_col not in df_dna_pred.columns or pred_col not in df_dna_pred.columns:
        print(f"Warning: Missing columns for DNA metrics ({obs_col}, {pred_col}). Returning NaN.")
        ...
        return {"SSE": np.nan, "R²": np.nan, "RMSE": np.nan, "MAE": np.nan,
                ...
                "NRMSE_mean": np.nan, "NRMSE_range": np.nan}

    # Filter out NaNs and the resuspended step
    df_metrics = df_dna_pred.dropna(subset=[obs_col, pred_col]).copy()

    # Filter out resuspended step if step_col exists
    if step_col in df_metrics.columns:
    ...
    if valid_mape_idx.any():
        mape = np.mean(np.abs((observed[valid_mape_idx] - predicted[valid_mape_idx]) /
                              observed[valid_mape_idx])) * 100.0
    else:
        mape = np.nan

    # --- Normalized RMSE ---
    # 1. Normalized by mean
    nrmse_mean = rmse / mean_observed if mean_observed > 1e-12 else np.nan

    # 2. Normalized by range
    obs_range = observed.max() - observed.min()
    nrmse_range = rmse / obs_range if obs_range > 1e-12 else np.nan
    ...
    return {
        ...
        "MAPE": mape,
        "Bias": bias,
        "NRMSE_mean": nrmse_mean,  # New - RMSE normalized by mean
        "NRMSE_range": nrmse_range,  # New - RMSE normalized by range
        "N_valid": n_valid
    }